...
- Supported accelerators: Intel® vRAN Dedicated Accelerator ACC100, Intel® vRAN Boost Accelerator V1 (VRB1), Intel® vRAN Boost Accelerator V2 (VRB2)
- OpenShift 4.12 or higher (see [release notes](sriov-fec-operator-release-notes.md) for the full compatibility matrix)
...
- When using `vfio-pci` as the PF driver (required for telemetry), the following kernel parameters must be set:
...
sriov-fec.v2.12.0   SR-IOV Operator for Wireless FEC Accelerators             2.12.0               Succeeded
```
...
> **Note:** Telemetry is **disabled by default** (since v2.10.0). To enable it, set the `SRIOV_FEC_METRIC_GATHER_INTERVAL` environment variable in the `manager` container of the `sriov-fec-controller-manager` deployment to a valid Go duration (e.g. `15s`, `1m`). Setting it to `0s` disables telemetry. See the [Telemetry section](sriov-fec-operator.md#telemetry) of the operator spec for details.

To enable telemetry at install time, set `SRIOV_FEC_METRIC_GATHER_INTERVAL` via the Subscription `config.env` field:
...
sriov-fec-subscription   sriov-fec   certified-operators   stable
```

Then delete the items and the namespace:

```shell
[user@ctrl1 /home]# oc delete csv sriov-fec.v2.12.0
