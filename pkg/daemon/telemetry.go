...
func readFileWithTelemetry(pciAddr string, log *logrus.Logger) ([]byte, error) {
	...
	err := wait.Poll(time.Millisecond*50, time.Second, func() (done bool, err error) {
		file, err = os.ReadFile(fmt.Sprintf("/var/log/pf_bb_cfg_%v.log", pciAddr))
		if err != nil {
			log.WithField("pciAddr", pciAddr).WithError(err).Warnf("failed to read pf_bb_config log")
			return false, nil
		}
		if len(file) == 0 {
			log.WithField("pciAddr", pciAddr).Warnf("file with telemetry has 0 bytes")
			return false, nil
		}
		return true, nil
	})
	if err != nil {
		log.WithField("pciAddr", pciAddr).WithError(err).Error("unknown error with file")
		return nil, err
	}
	return file, nil
}
...
func clearLog(pciAddr string) error {
	fileName := fmt.Sprintf("/var/log/pf_bb_cfg_%v.log", pciAddr)
